...
#-------------------------------------------------------------------------------------------------
input_volpath = '/scra3/ft21894/radx/bin/output_0713/20150715/03000400/'
input_volpath2 = '/scra3/ft21894/radx/bin/output_0713/20150715/03000400/'
input_volpath3 = '/scra3/ft21894/radx/bin/output_0713/20150715/03000400/'
radarvollist = sorted(glob.glob(input_volpath+'*KGLD*'))
radarvollist2 = sorted(glob.glob(input_volpath2+'*KDDC*'))
radarvollist3 = sorted(glob.glob(input_volpath3+'*SPOL*'))
output_gridpath = '/scra6/ft21894/radartrunk/0715/'
...
#	print(i)
	...
	#Force radar fields to be consistent between radar objects (SPolKa -> NEXRAD)
	#------------------------------------------------------------------------------------------------
	if research=='SPOL':
		ref = deepcopy(base3.fields['DBZ_F']['data'])
		zdr = deepcopy(base3.fields['ZDR_F']['data'])
		rho = deepcopy(base3.fields['RHOHV_F']['data'])
		kdp = deepcopy(base3.fields['KDP_F']['data'])
		rain = deepcopy(base3.fields['RATE_HIDRO']['data'])
		snr = deepcopy(base3.fields['SNRHC_F']['data'])
		ncp = deepcopy(base3.fields['NCP_F']['data'])
		width = deepcopy(base3.fields['WIDTH_F']['data'])
		base3.add_field_like('DBZ_F','REF',ref,replace_existing=True)
		base3.add_field_like('ZDR_F','ZDR',zdr,replace_existing=True)
		base3.add_field_like('SNRHC_F','SNR',snr,replace_existing=True)
		base3.add_field_like('RHOHV_F','RHO',rho,replace_existing=True)
		base3.add_field_like('KDP_F','KDP',kdp,replace_existing=True)
		base3.add_field_like('NCP_F','NCP',ncp,replace_existing=True)
		base3.add_field_like('RATE_HIDRO','rain',rain,replace_existing=True)
		base3.add_field_like('WIDTH_F','SW',width,replace_existing=True)
		del ref,zdr,rho,kdp,rain,snr,ncp

	# Read in sounding and interpolate onto radar gates
	#-------------------------------------------------------------------------------------------------
	date_name = datetime(2015,7,15,0)
	station_name = 'KDDC' #KOX
	...
	base = retrieval.retrieve_dsd(base,gatefilter=gatefilter,kdp_field='KDP',\
			zdr_field='zdr_acorr',refl_field='REF')
	...
	grid_limits = ((0, 20000), (-200000.0, 200000.0), (-200000.0, 200000.0))
	grid_roi    = 2000
	...
	out_fn  = '_'.join([base.metadata['instrument_name'],base2.metadata['instrument_name'],base3.metadata['instrument_name'],
		dt.strftime('%Y%m%d_%H%M%S'),'grid_mass_comp_v3']) + '.nc'
	...
	del grid_dis,base,base2,base3,snr,snr2,zdr_corr,zdr_corr2,zdr_corr3,unfphidic,unfphidic2,\
			kdp,fdp,sdp,kdp2,fdp2,sdp2
